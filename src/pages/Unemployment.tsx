...
export default function Unemployment() {
  ...
  const fetchServices = async () => {
    try {
      const data = await getServicesByCategory('البطالة')
      setServices(data || [])
    } catch (error) {
    ...
    <div className="min-h-screen">
      ...
      <section className="bg-gradient-to-b from-primary-50 to-white px-6 py-16 lg:px-12">
        <div className="container mx-auto text-center">
          <UserMinus className="mx-auto mb-6 h-16 w-16 text-primary-500" />
          <h1 className="mb-4 text-h1 font-bold text-neutral-900">خدمات البطالة</h1>
          <p className="mx-auto max-w-2xl text-body-large text-neutral-700">
            التسجيل في منحة البطالة للمستحقين
          </p>
        </div>
      </section>
